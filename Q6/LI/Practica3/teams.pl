...
incompatibleWorkers(W1,W2) :- workerScore(W1, S1), workerScore(W2, S2), maxScore(M), SUM is S1 + S2, SUM > M. %% Complete this!
...
writeClauses :-
    %%.... %% Complete this!  
    % 1. Size of the team beween min i maxSize
    correctSize,
    % 2. The workers must be compatible
    compatibleWorkers,
    % 3. All workers must be in a team
    allWorkersInTeam,
    % 4. There are exactly numTeams
    exactlyNumTeams,  %Diria que aquesta no fa falta
...
correctSize :-
        ...
        team(T), findall(wt(W, T), worker(W), L),
        atLeast(Min, L), atMost(Max, L), fail.
correctSize.

compatibleWorkers :-
        worker(W1), worker(W2), W1 < W2, %Per evitar clausules duplicades
        incompatibleWorkers(W1, W2), team(T),
        writeOneClause([-wt(W1, T), -wt(W2, T)]), fail.
compatibleWorkers.

allWorkersInTeam :-
        worker(W), findall(wt(W,T), team(T), L),
        exactly(1, L), fail.
allWorkersInTeam.

exactlyNumTeams :-
        numTeams(N), findall(T, team(T), L),
        length(L, N), fail.
exactlyNumTeams.
