...
satVariable(startFromEnd(B,D1,D2,C)) :- bus(B), consecutiveDays(D1,D2), city(C).
city(C) :- trip(C1-C2), (C = C1 ; C = C2).
...
writeClauses :- 
    ...
    %1. A bus cannot do more than maxDistance in two consecutive days
    lessMaxDistance,
    %2. Each travel have a frequency
    minFrequency,

    connectConsecutiveDays,
...
lessMaxDistance :-
        ...
        findall((C1-C2)-(C2-C3), (trip(C1-C2), trip(C2-C3), tooLongDist(C1-C2-C3)), L),
        member((C1-C2)-(C2-C3), L),
...
connectConsecutiveDays :-
    bus(B), consecutiveDays(D1, D2), city(C2),
    findall(bdcc(B,D1,C1,C2), trip(C1-C2), EndsAtC2),
    findall(bdcc(B,D2,C2,C3), trip(C2-C3), StartsFromC2),
    member(bdcc(B,D1,C1,C2), EndsAtC2),
    writeOneClause([-bdcc(B,D1,C1,C2) | StartsFromC2]),
    fail.
        
connectConsecutiveDays.

% Ensure each traject happens at most once per day
atmostOneTripPerDay :-
    day(D), trip(C1-C2),
    findall(bdcc(B,D,C1,C2), bus(B), L),
    atMost(1, L),
    fail.
atmostOneTripPerDay.

% Link dcc variables with bdcc variables
linkDccVariables :-
    day(D), trip(C1-C2),
    findall(bdcc(B,D,C1,C2), bus(B), L),
    expressOr(dcc(D,C1,C2), L),
    fail.
linkDccVariables.
...
main(InputFile):-
        ...
        shell('~/Escriptori/Uni/Q6/LI/kissat/build/kissat -v infile.cnf > model', Result),  % if sat: Result=10; if unsat: Result=20.
        treatResult(Result),!.
