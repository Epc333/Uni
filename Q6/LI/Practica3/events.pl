...
satVariable( ed(E,D) ) :- event(E),  eventDays(E,Ds), member(D,Ds).  %% Complete this!
satVariable( em(E,M) ) :- event(E), eventModerators(E, Ms), member(M, Ms).
...
writeClauses :-  
    %.... %% Complete this!  
    % 1. Each event only one day
    eventOneDay,
    % 2. Each event only one moderator
    eventOneModerator,
    % 3. Cannot be more than maxEventsDay
    maxEventsDay,
    % 4. One moderator cannot work more than maxDaysPerModerator.
    maxDaysModerator,
    % 5. We have to make clauses for restrictions the event E, take place on day D, with moderator M, and modreator M works on day D
    makeClauses,
    true,!.
...
eventOneDay :- 
        event(E),
        findall(ed(E,D), (eventDays(E,Ds), member(D, Ds)), L),
        exactly(1, L), fail.
eventOneDay.

eventOneModerator :-
    event(E),
    findall(em(E,M), (eventModerators(E, Ms), member(M, Ms)), L),
    exactly(1, L), fail.
eventOneModerator.

maxEventsDay :-
        maxEventsPerDay(Max), day(D),
        findall(ed(E, D), (event(E), eventDays(E,Ds), member(D,Ds)), L),
        atMost(Max, L), fail.
maxEventsDay.

maxDaysModerator :-
        maxDaysPerModerator(Max), moderator(M),
        findall(md(M, D), day(D), L),
        atMost(Max, L), fail.
maxDaysModerator.

makeClauses :-
    event(E), eventDays(E,Ds), member(D,Ds),
    eventModerators(E,Ms), member(M,Ms),
