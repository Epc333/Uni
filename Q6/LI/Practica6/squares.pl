...
insideBigSquare(_,[], []) :- !.
insideBigSquare(Big, [Sx | S], [X | L]) :-
    X + Sx - 1 #=< Big,
    insideBigSquare(Big, S, L).


nonoverlapping([], [], []) :- !.
nonoverlapping([Sx | Sides], [Rx | R], [Cx | C]) :-
    nonoverlappingAux(Sx, Rx, Cx, Sides, R, C),
    nonoverlapping(Sides, R, C).

nonoverlappingAux(_, _, _, [], [], []) :- !.
nonoverlappingAux(S, R, C, [OneSide | Sides], [OneRow | Rows], [OneColumn | Columns]) :-
    R + S #=< OneRow #\/ OneRow + OneSide #=< R #\/ 
    C + S #=< OneColumn #\/ OneColumn + OneSide #=< C,
    nonoverlappingAux(S, R, C, Sides, Rows, Columns).
