...
writeClauses(NumUnAssigned):-
    ...
    exactlyOneGangsterPerRoom(NumAssigned),
    noEnemis(NumAssigned),
    %...
...
exactlyOneGangsterPerRoom(NumAssigned) :-
        room(R),
        findall(gr(G,R),(gangster(G), G =< NumAssigned), Lits),
        atMost(1, Lits), fail.
exactlyOneGangsterPerRoom(_).
%...
noEnemis(NumAssigned) :-
        gangster(G1), gangster(G2), G1 =< NumAssigned, G2 =< NumAssigned, enemies(G1, G2), room(R1), room(R2), neighborRooms(R1,R2),
        writeOneClause([-gr(G1,R1), -gr(G2,R2)]), fail.
noEnemis(_).
...
costOfThisSolution(M,Cost):-  findall(G, member(gr(G,_), M), Lits), numGangsters(N), length(Lits, Num), Cost is N - Num.
