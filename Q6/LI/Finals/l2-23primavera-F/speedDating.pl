...
noIncompatibleDatesAtSameSlot(MaxSlots):- 
        ...
        shareParticipant(P1-P2, P3-P4), % slot(S),
...
relatePSwithPPS(MaxSlots):- 
        participant(P), between(1,MaxSlots,S), 
        findall(pps(P-P1,S), date(P-P1), Lits1), 
        findall(pps(P1-P,S), date(P1-P), Lits2), 
        append(Lits1, Lits2, Lits), 
        expressOr(ps(P,S), Lits), 
        fail.
relatePSwithPPS(_).

restEveryKConsecutiveDates(MaxSlots):- 
        maxConsecutiveDates(K), K1 is K+1, 
        kConsecutiveSlots(MaxSlots, K1, L),
        participant(P), 
        findall(-ps(P,S), member(S,L), Lits), 
        writeOneClause(Lits),
        fail.
...
maxDatesAtTheSameSlot(MaxSlots):- 
        numTables(Max),
        between(1, MaxSlots, S),
        findall(ps(P, S), participant(P), Lits), 
        Aux is 2 * Max,
        atMost(Aux, Lits), fail.
maxDatesAtTheSameSlot(_).

betweenVipsBefore(MaxSlots):-
        vips(LPV), 
        date(P1-P2), member(P1,LPV), member(P2,LPV), 
        date(P3-P4), \+ member(P3,LPV), \+ member(P4,LPV),
        between(1,MaxSlots,S34), S34_1 is S34+1, 
        between(S34_1,MaxSlots,S12), 
...
costOfThisSolution(M,Cost):- findall(S, member(pps(_-_,S), M), Lits), max_list(Lits, Cost), !.
