...
golomb(N, M) :-
    ...
    last(V,M),
    increasing(V),
    findall([[I, J], [K, L]], (between(1, N, I), I1 is I+1, between(I1, N, J), between(I, N, K), K1 is K+1, between(K1, N, L), ((I #= K, J #< L); I #< K)), Pairs),
    diff(Pairs,V), 
    label(V),
    write(V), nl.
...
diff([],_).
diff([[I,J],[K,L]|LP],LV) :- nth1(I,LV,E1), nth1(J,LV,E2), nth1(K,LV,E3), nth1(L,LV,E4), E2 - E1 #\= E4 - E3, diff(LP,LV).
...
% diff([], _).
% diff([ [[I, J], [K, L]] | R], V) :-
%     nth1(I, V, VI), nth1(J, V, VJ),
%     nth1(K, V, VK), nth1(L, V, VL),
%     VJ - VI #\= VL - VK,
%     diff(R, V).
% 
% increasing([_]) :- !.
