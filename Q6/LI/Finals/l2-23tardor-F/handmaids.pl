...
forbiddenGroups :-  %groups that never should go together in any row, any day
    forbidden(L), member([H1,H2,H3],L), day(D), row(R),
...
relate_together_coincide2 :-
    day(D),
    handmaid(H1), handmaid(H2), H1 < H2,
    findall(together(H1, H2, D, R), row(R), Lits),
    expressOr(coincide2(H1, H2, D), Lits),
    fail.
relate_together_coincide2.
...
relate_coincide2_coincide3 :-
    ...
    expressAnd(coincide3(H1, H2, H3, D), [coincide2(H1, H2, D), coincide2(H2, H3, D)]),
    fail.
relate_coincide2_coincide3.

mandatoryGroups :-
    mandatory(L1), member([H1,H2,H3],L1),
    findall(coincide3(H1, H2, H3, D), day(D), Lits),
    atLeast(1, Lits),
    fail.
mandatoryGroups.
