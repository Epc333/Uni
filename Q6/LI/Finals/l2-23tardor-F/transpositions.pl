...
oneStep(Cost, InitialState, NextState) :-
    member(X, InitialState),
    member(Y, InitialState),
    X \= Y,
    nth1(PosX, InitialState, X),
    nth1(PosY, InitialState, Y),
    Cost is abs(PosX-PosY),
    append(L1, L2, InitialState),
    length(L1, PosX),
    
    append(L3, [X], L1),
    append(L4, [Y|Rest], L2),
    append(L3, [Y|L4], L5),
    append(L5, [X| Rest], NextState).
