...
eachDayEnoughWorkers:- 
        day(D),
        findall(works(W,D), worker(W), Lits),
        numWorkersNeededForDay(D, Num),
        atLeast(Num, Lits),
        fail.
eachDayEnoughWorkers.

blocked:-
        worker(W), day(D), isBlocked(W,D),
        writeOneClause([-works(W, D)]),
        fail.
blocked.

atMostThreeConsecutiveDays:-
        worker(W), fourConsecutiveDays(D1,D2,D3,D4),
        writeOneClause([-works(W,D1), -works(W,D2), -works(W,D3), -works(W,D4)]),
        fail.
atMostThreeConsecutiveDays.
...
defineNoSupervisor:- 
        day(D), 
        findall(works(W, D), (worker(W), supervisor(W)), Lits),
        writeOneClause([noSupervisor(D)|Lits]), fail.
defineNoSupervisor.

maxDaysWithoutSupervis(Max):-
        findall(noSupervisor(D), day(D), Lits), atMost(Max, Lits), fail.
...
%
costOfThisSolution(M,Cost):- findall(D, (member(noSupervisor(D), M)), Lits), length(Lits, Cost).
