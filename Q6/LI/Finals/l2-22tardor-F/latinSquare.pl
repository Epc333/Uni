...
example1 :- latin([2,_,_,3,3,_,_,_,_,_,_,1,_,_,4,_]).
...
matrixByRows(_, [], []).
matrixByRows(N, L, Matrix) :-
    append(Row, Rest, L),
    length(Row, N),
    append([Row], MatrixAux, Matrix),
    matrixByRows(N, Rest, MatrixAux).

constraintsFromSubLists([]) :- !.
constraintsFromSubLists([Row | Matrix]) :-
    all_distinct(Row),
