...
%2: Constraints:
    insideMaximumMakespan(StartSlots, Durations, Limit),% ... COMPLETE
    precedenceConstraints(StartSlots, Durations, Precedences),% ... COMPLETE
...
insideMaximumMakespan(SX, DX, Limit) :- % COMPLETE
    defineEndSlots(SX, DX, End),
    maxExprOfEndSlots(End, X),
    X #=< Limit.

precedenceConstraints(_, _, []).
precedenceConstraints(S, D, [X-Y | P]) :- % COMPLETE
    defineEndSlots(S, D, E),
    nth1(X, E, EL1), nth1(Y, S, EL2), EL2 #> EL1, precedenceConstraints(S,D,P).
...
maxExprOfEndSlots([X|L], max(X,Y)) :- % COMPLETE
    maxExprOfEndSlots(L, Y).
