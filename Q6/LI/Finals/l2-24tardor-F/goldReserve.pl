...
eachBarOneC(MaxContainers) :-
    bar(I), 
    findall(bc(I, J), (container(J), J =< MaxContainers), Lits),
    exactly(1, Lits),
    fail.
eachBarOneC(_).
...
order(MaxContainers) :-
    Aux is MaxContainers - 1,
    between(1, Aux, J),
    container(J),
    J2 is J + 1,
    container(J2),
    writeOneClause([c(J), -c(J2)]), fail.
order(_).
...
costOfThisSolution(M,Cost) :- findall( J, member(bc(_,J), M), Lits), max_list(Lits, Cost).
