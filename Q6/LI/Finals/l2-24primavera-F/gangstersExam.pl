...
%% ADDED FOR THIS EXAM:
threeConsecutiveHours(H1,H2,H3) :- between(1,69,H1), H2 is H1 + 1, H3 is H1 + 2.    %% COMPLETE 1/5
...
%%  NEW FOR THIS EXAM:
twoRestHoursBetweenTasks :-
    gangster(G), threeConsecutiveHours(H1, H2, H3),
    available(G, H1), available(G, H3),
...
relateDoesVarsWithDoesTask :-
    ...
    expressOr(doesTask(G,T), Lits), fail.  %% COMPLETE 3/5
relateDoesVarsWithDoesTask.
 
someGangstersAtMostTwoDifferentTasks :-
    maxTwoTaskGangsters(Gang), member(G, Gang),
    findall(doesTask(G, T), task(T), Lits),
    atMost(2, Lits), fail.  %% COMPLETE 4/5
someGangstersAtMostTwoDifferentTasks.

someGangstersHaveProhibitedTasks :-
    notAllowedTask(G, T),
    writeOneClause([-doesTask(G,T)]), fail.  %% COMPLETE 5/5
someGangstersHaveProhibitedTasks.
