...
relateClauses :-
        ...
        expressOr(towerVillage(V), Lits), fail.
relateClauses.
...
importantVillage :-
        ...
        writeOneClause([towerVillage(V)]), fail.
importantVillage.

noVillageNoTower :-
        position(I, J),
        not(posVillage(_, I, J)),
        writeOneClause([-towerPos(I, J)]), fail.
noVillageNoTower.
...
costOfThisSolution(M,Cost):- 
        findall(_, member(towerPos(_, _), M), Lits), length(Lits, Cost), !.
