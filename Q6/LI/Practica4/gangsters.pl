...
satVariable( gangsterWork(G, H)) :- gangster(G), hour(H).
...
writeClauses(MaxConsecutiveHours) :-
    ...
    relateClauses,        
    noConsecutive,        
    maxMaxConsecutiveHours(MaxConsecutiveHours), 
    true,!.
...
relateClauses :-
        ...
        expressOr(gangsterWork(G, H), Lits), fail.
relateClauses.

gangsterNoWork :-
        gangster(G), hour(H),
        blocked(G, H),
        writeOneClause( [-gangsterWork(G, H)] ), fail.
gangsterNoWork.
...
maxMaxConsecutiveHours(MaxConsecutiveHours) :-
        gangster(G), task(T), 
        hour(Start), hour(End), End is Start + MaxConsecutiveHours,
        findall(does(G, T, H), between(Start, End, H), Lits),
        negateAll(Lits, NLits), atLeast(1, NLits), fail.
maxMaxConsecutiveHours(_).
...
% Here the sort predicate is used to remove repeated elements of the list:
 costOfThisSolution(M,Cost):- 
         ...
         gangster(G),
         listOfWorkedHours(G, M, L),
         existsSublistWithKConsecutiveHours(Cost, L),
         !.
 ...
 existsSublistWithKConsecutiveHours(Cost, L) :-
         append([_, LK, _], L), length(LK, Cost), 
         consecutiveHours(LK).
 
 consecutiveHours([]).
 consecutiveHours([_]) :- !.
 consecutiveHours([F, S | R]) :- S is F + 1, consecutiveHours([S | R]).
