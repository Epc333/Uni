  ...
  if (ctx->expr().size() > 0) {
    for (auto ex : ctx->expr()){
      ...
      if (not Types.isErrorTy(t) and not Types.isNumericTy(t))
        error = true;
      if (not Types.isErrorTy(t) and Types.isFloatTy(t))
        real = true;
    }
  }
  if (error) Errors.incompatibleOperator(ctx->SUM()->getSymbol());
  if (real) putTypeDecor(ctx, Types.createFloatTy());
...
std::any TypeCheckVisitor::visitFilterExpr(AslParser::FilterExprContext *ctx) {
  ...
  if (not Types.isErrorTy(t1) and 
      (not Types.isArrayTy(t1) or 
       not Types.isBooleanTy(Types.getArrayElemType(t1)))) {
    validTypes = false;
  }
  
  if (not Types.isErrorTy(t0) and not Types.isErrorTy(t1) and
      Types.isArrayTy(t0) and Types.isArrayTy(t1) and
      Types.getArraySize(t0) != Types.getArraySize(t1)) {
    validTypes = false;
  }
  
  if (not Types.isErrorTy(t2) and 
      (not Types.isFunctionTy(t2) or
       not Types.isBooleanTy(Types.getFuncReturnType(t2)) or
       Types.getNumOfParameters(t2) != 1)) {
    validTypes = false;
  }
  
  if (not Types.isErrorTy(t0) and not Types.isErrorTy(t2) and
      Types.isArrayTy(t0) and Types.isFunctionTy(t2) and
      Types.getNumOfParameters(t2) == 1 and
      not Types.copyableTypes(Types.getParameterType(t2, 0), Types.getArrayElemType(t0))) {
    validTypes = false;
  }
  
  if (not validTypes) {
    Errors.incompatibleOperator(ctx->FILTER()->getSymbol());
  }
