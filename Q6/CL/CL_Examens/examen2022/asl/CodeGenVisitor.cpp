...
std::any CodeGenVisitor::visitVariable_decl(AslParser::Variable_declContext *ctx) {
  ...
  for (auto id : ctx->ID()){
    ...
    } else if (Types.isMatrixTy(t1)) { // Afegir suport per matrius
      vars.push_back(var{id->getText(), Types.to_string(Types.getMatrixElemType(t1)), size});
    }  else
...
std::any CodeGenVisitor::visitAssignStmt(AslParser::AssignStmtContext *ctx) {
  ...
  // COPIA DE ARRAYS i MATRIUS: a = b
  if ((Types.isArrayTy(t1) && Types.isArrayTy(t2)) || (Types.isMatrixTy(t1) && Types.isMatrixTy(t2))) {
    std::string temp1 = "%" + codeCounters.newTEMP();
    ...
    std::string limit = "%"+codeCounters.newTEMP(); // mida array/matrix
    ...
    // Inicialitzar bucle - usar getSizeOfType que funciona tant per arrays com matrius
    std::size_t totalSize;
    if (Types.isArrayTy(t1)) {
      totalSize = Types.getArraySize(t1);
    } else {
      totalSize = Types.getSizeOfType(t1); // Per matrius, retorna files * columnes
    }
    
    code = code || instruction::ILOAD(limit, std::to_string(totalSize)); // Límit del bucle
    code = code || instruction::ILOAD(ivar, "0"); // i = 0 
    code = code || instruction::ILOAD(inc, "1");  // increment es 1
    
    // Bucle que recorre l'array/matriu
...
std::any CodeGenVisitor::visitLeft_expr(AslParser::Left_exprContext *ctx) {
  ...
  if (ctx->expr(1)){
    // Matrix access case: ident[expr, expr]
    std::string  addr1 = codAts.addr;   // Address of the ident
    instructionList & code1 = codAts.code;

    CodeAttribs && codAt2 = std::any_cast<CodeAttribs>(visit(ctx->expr(0))); // Row index
    instructionList & code2 = codAt2.code;
    std::string     addr2 = codAt2.addr;  

    CodeAttribs && codAt3 = std::any_cast<CodeAttribs>(visit(ctx->expr(1))); // Column index
    instructionList & code3 = codAt3.code;
    std::string     addr3 = codAt3.addr;  

    instructionList  code = code1 || code2 || code3;

    // Calculate matrix offset: row * numCols + col
    TypesMgr::TypeId t = getTypeDecor(ctx->ident());
    int numCols = Types.getMatrixCols(t);

    std::string temp2 = "%" + codeCounters.newTEMP();
    std::string offset = "%" + codeCounters.newTEMP();
    std::string nCols = "%" + codeCounters.newTEMP();
    std::string rowOffset = "%" + codeCounters.newTEMP();

    code = code || instruction::ILOAD(nCols, std::to_string(numCols));
    code = code || instruction::MUL(rowOffset, addr2, nCols);  // row * numCols
    code = code || instruction::ADD(offset, rowOffset, addr3); // row * numCols + col
    ...
    codAts = CodeAttribs(temp2, offset, code);
  }
  else if(ctx->expr(0)){ // Si es tracta d'un array

    std::string  addr1 = codAts.addr;   // Adreça de l'ident
    instructionList & code1 = codAts.code;

    CodeAttribs && codAt2 = std::any_cast<CodeAttribs>(visit(ctx->expr(0)));
    instructionList & code2 = codAt2.code;
...
std::any CodeGenVisitor::visitFactorial(AslParser::FactorialContext *ctx){
  ...
  // Variables for the factorial calculation
  std::string ivar = "%"+codeCounters.newTEMP();     // counter
  std::string limit = "%"+codeCounters.newTEMP();    // n (input value)
  std::string cond = "%"+codeCounters.newTEMP();     // condition
  std::string result = "%"+codeCounters.newTEMP();   // factorial result
  std::string inc = "%"+codeCounters.newTEMP();      // increment
  ...
  std::string negCheck = "%"+codeCounters.newTEMP();
  std::string skipHalt = "skipHalt" + codeCounters.newLabelIF();

  // Initialize factorial calculation
  code = code || instruction::LOAD(limit, addr1);    // limit = n
  code = code || instruction::ILOAD(zero, "0");      // zero = 0

  // Check if n < 0, if so, halt with error
  code = code || instruction::LT(negCheck, limit, zero);  // negCheck = (n < 0)
  code = code || instruction::FJUMP(negCheck, skipHalt);  // if (!negCheck) skip halt
  code = code || instruction::HALT(code::INVALID_INTEGER_OPERAND);  // halt with error
  code = code || instruction::LABEL(skipHalt);

  // Initialize factorial calculation
  code = code || instruction::ILOAD(ivar, "1");      // i = 1 
  code = code || instruction::ILOAD(inc, "1");       // increment = 1
  code = code || instruction::ILOAD(result, "1");    // result = 1

  // Loop
  code = code || instruction::LABEL(labelLoop);               
  code = code || instruction::LE(cond, ivar, limit);          // cond = i <= limit
  code = code || instruction::FJUMP(cond, labelEnd);          // if (!cond) goto end
  code = code || instruction::MUL(result, result, ivar);      // result = result * i
  code = code || instruction::ADD(ivar, ivar, inc);           // i++
  code = code || instruction::UJUMP(labelLoop);               // goto loop
  code = code || instruction::LABEL(labelEnd);

  // Return the result in a new temp variable
  std::string temp = "%"+codeCounters.newTEMP();
  code = code || instruction::LOAD(temp, result);
...
std::any CodeGenVisitor::visitMatAccess(AslParser::MatAccessContext *ctx){
  DEBUG_ENTER();
  instructionList     code;
  ...
  instructionList &   code1 = codAt1.code;
  TypesMgr::TypeId t = getTypeDecor(ctx->ident());
  int N = Types.getMatrixCols(t);

  CodeAttribs     && codAt2 = std::any_cast<CodeAttribs>(visit(ctx->expr(0)));
  std::string         addr2 = codAt2.addr;
  instructionList &   code2 = codAt2.code;
 
  CodeAttribs     && codAt3 = std::any_cast<CodeAttribs>(visit(ctx->expr(1)));
  std::string         addr3 = codAt3.addr;
  instructionList &   code3 = codAt3.code;
  ...
  // Calculate the offset: i * N + j
  std::string offset = "%"+codeCounters.newTEMP();
  std::string nCols = "%"+codeCounters.newTEMP();
  std::string temp1 = "%"+codeCounters.newTEMP();
  
  code = code || instruction::ILOAD(nCols, std::to_string(N));  // Load number of columns
  code = code || instruction::MUL(temp1, addr2, nCols);        // temp1 = i * N
  code = code || instruction::ADD(offset, temp1, addr3);       // offset = i * N + j
  
  // Load the matrix element at calculated offset
  std::string temp2 = "%"+codeCounters.newTEMP();
  if (Symbols.isParameterClass(ctx->ident()->getText())){
    code = code || instruction::LOAD(temp2, addr1);  // Get base address for parameter
  } else {
    code = code || instruction::ALOAD(temp2, addr1); // Get base address for local variable
  }
  code = code || instruction::LOADX(temp, temp2, offset);  // temp = matrix[offset]
