...
std::any TypeCheckVisitor::visitLeft_expr(AslParser::Left_exprContext *ctx) {
  ...
  if (ctx->expr(1)) {  // Matrix access: ident '[' expr ',' expr ']'
    if (not Types.isErrorTy(t1) and not Types.isMatrixTy(t1)) {
      ...
      TypesMgr::TypeId t = Types.getMatrixElemType(t1);
      putTypeDecor(ctx, t);
      bool b = getIsLValueDecor(ctx->ident());
      putIsLValueDecor(ctx, b);
    }
    ...
    if ((not Types.isErrorTy(t2)) and (not Types.isIntegerTy(t2)))
      Errors.nonIntegerIndexInMatrixAccess(ctx);

    visit(ctx->expr(1));
    TypesMgr::TypeId t3 = getTypeDecor(ctx->expr(1));
    if ((not Types.isErrorTy(t3)) and (not Types.isIntegerTy(t3)))
      Errors.nonIntegerIndexInMatrixAccess(ctx);
    
  }
  else if (ctx->expr(0)){  // Array access: ident '[' expr ']'
...
std::any TypeCheckVisitor::visitFactorial(AslParser::FactorialContext *ctx){
  ...
  if (not Types.isErrorTy(t) and not Types.isIntegerTy(t))
    Errors.incompatibleOperator(ctx->FACT()->getSymbol());
...
std::any TypeCheckVisitor::visitMatAccess(AslParser::MatAccessContext *ctx) {
  DEBUG_ENTER();
  ...
  // Si no és de tipus matrix i no és de tipus error -> mostrem error
  if(not Types.isErrorTy(t1) and not Types.isMatrixTy(t1)){
    Errors.nonMatrixInMatrixAccess(ctx);
    putTypeDecor(ctx, Types.createErrorTy());
    putIsLValueDecor(ctx, false);
  } else {
    // Si és una matrix, el tipus és el tipus dels elements
    if (Types.isMatrixTy(t1)) {
      TypesMgr::TypeId tElem = Types.getMatrixElemType(t1);
      putTypeDecor(ctx, tElem);
      putIsLValueDecor(ctx, true); // És modificable
    } else {
      putTypeDecor(ctx, Types.createErrorTy());
      putIsLValueDecor(ctx, false);
    }
  ...
  // Obtenim el tipus de les expressions d'índex
  visit(ctx->expr(0)); // primera dimensió
  TypesMgr::TypeId t2 = getTypeDecor(ctx->expr(0));
  visit(ctx->expr(1)); // segona dimensió  
  TypesMgr::TypeId t3 = getTypeDecor(ctx->expr(1));

  // Si no són enters, error d'accés amb índex no enter
  if ((not Types.isErrorTy(t2)) and (not Types.isIntegerTy(t2)))
...
std::any TypeCheckVisitor::visitUnary(AslParser::UnaryContext *ctx) {
  ...
  if (Types.isErrorTy(t1))  // Si l'expr és error, decorem l'unary com error
    ...
    if (not Types.isBooleanTy(t1)){
      ...
      putTypeDecor(ctx, Types.createBooleanTy()); // Cal decorar tota l'expr unary com error
    }else{
