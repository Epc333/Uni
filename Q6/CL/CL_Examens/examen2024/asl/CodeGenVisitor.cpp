...
/*std::any CodeGenVisitor::visitSwapStmt(AslParser::SwapStmtContext *ctx) {
  ...
  CodeAttribs  && codAtsE0 = std::any_cast<CodeAttribs>(visit(ctx->left_expr(0)));
  std::string     addr0    = codAtsE0.addr;
  instructionList & code0  = codAtsE0.code;
  std::string       offs0  = codAtsE0.offs;


  CodeAttribs  && codAtsE1 = std::any_cast<CodeAttribs>(visit(ctx->left_expr(1)));
  std::string     addr1    = codAtsE1.addr;
  instructionList & code1  = codAtsE1.code;
  std::string       offs1  = codAtsE1.offs;
  ...
  if (Types.isArrayTy(t0) and Types.isArrayTy(t1) and offs0 == "" and offs1 == ""){
    // Array swap case - para arrays locales, addr0 y addr1 ya son las direcciones base
    std::string baseAddr0 = addr0;
    std::string baseAddr1 = addr1;
    
    // Loop variables
    std::string ivar = "%"+codeCounters.newTEMP();
    std::string limit = "%"+codeCounters.newTEMP();
    std::string cond = "%"+codeCounters.newTEMP();
    std::string tempVal0 = "%"+codeCounters.newTEMP();
    std::string tempVal1 = "%"+codeCounters.newTEMP();
    ...
    // Initialize loop
    code = code || instruction::ILOAD(limit, std::to_string(Types.getArraySize(t0)));
    code = code || instruction::ILOAD(ivar, "0");
    code = code || instruction::ILOAD(inc, "1");
    
    // Swap loop
    code = code || instruction::LABEL(labelLoop);
    code = code || instruction::LT(cond, ivar, limit);
    code = code || instruction::FJUMP(cond, labelEnd);
    code = code || instruction::LOADX(tempVal0, baseAddr0, ivar);   
    code = code || instruction::LOADX(tempVal1, baseAddr1, ivar);   
    code = code || instruction::XLOAD(baseAddr0, ivar, tempVal1);   
    code = code || instruction::XLOAD(baseAddr1, ivar, tempVal0);   
    code = code || instruction::ADD(ivar, ivar, inc);
    code = code || instruction::UJUMP(labelLoop);
    code = code || instruction::LABEL(labelEnd);
  }else {

    std::string temp0 = "%" + codeCounters.newTEMP();
    std::string temp1 = "%" + codeCounters.newTEMP();
    
    // Cargar el valor de la primera variable en temp1
    if (offs0 != "") { 
      code = code || instruction::LOADX(temp0, addr0, offs0);
    } else {
      code = code || instruction::LOAD(temp0, addr0);
    }
    
    // Cargar el valor de la segunda variable en temp2
    if (offs1 != "") { 
      code = code || instruction::LOADX(temp1, addr1, offs1);
    } else { 
      code = code || instruction::LOAD(temp1, addr1);
    }
    
    // Intercambiar: guardar temp1 en la primera variable
    if (offs0 != "") { // Si es un array
      code = code || instruction::XLOAD(addr0, offs0, temp1);
    } else { // Si es una variable simple
      code = code || instruction::LOAD(addr0, temp1);
    }
    
    // Intercambiar: guardar temp0 en la segunda variable
    if (offs1 != "") { // Si es un array
      code = code || instruction::XLOAD(addr1, offs1, temp0);
    } else { // Si es una variable simple
      code = code || instruction::LOAD(addr1, temp0);
    }
  }
...
std::any CodeGenVisitor::visitSwapStmt(AslParser::SwapStmtContext *ctx) {
  ...
  CodeAttribs     && codAtExpr1 = std::any_cast<CodeAttribs>(visit(ctx->left_expr(0)));
  std::string         addrExpr1 = codAtExpr1.addr;
  std::string         offs1     = codAtExpr1.offs;
  instructionList &   codeExpr1 = codAtExpr1.code;

  CodeAttribs     && codAtExpr2 = std::any_cast<CodeAttribs>(visit(ctx->left_expr(1)));
  std::string         addrExpr2 = codAtExpr2.addr;
  std::string         offs2     = codAtExpr2.offs;
  instructionList &   codeExpr2 = codAtExpr2.code;

  instructionList     code = codeExpr1 || codeExpr2;
  ...
  if (offs1 != "" && offs2 != "") {
    ...
    // temp = x[k]
    code = code || instruction::LOADX(temp, addrExpr1, offs1);
    // tempY = y[z]
    code = code || instruction::LOADX(tempY, addrExpr2, offs2);
    // x[k] = tempY
    code = code || instruction::XLOAD(addrExpr1, offs1, tempY);
    // y[z] = temp
    code = code || instruction::XLOAD(addrExpr2, offs2, temp);
  }
  // swap x[k]  y
  else if (offs1 != "") {
    // temp = x[k]
    code = code || instruction::LOADX(temp, addrExpr1, offs1);
    // x[k] = y
    code = code || instruction::XLOAD(addrExpr1, offs1, addrExpr2);
    // y = temp
    code = code || instruction::LOAD(addrExpr2, temp);
  }
  // swap x y[z]
  else if (offs2 != "") {
    std::string tempY = "%"+codeCounters.newTEMP();
    // temp = x
    code = code || instruction::LOAD(temp, addrExpr1);
    // tempY = y[z]
    code = code || instruction::LOADX(tempY, addrExpr2, offs2);
    // x = tempY
    code = code || instruction::LOAD(addrExpr1, tempY);
    // y[z] = temp
    code = code || instruction::XLOAD(addrExpr2, offs2, temp);
  }
  // COPIA DE ARRAYS
  else if (Types.isArrayTy(t0) && Types.isArrayTy(t1)) {
    std::string tempX = "%" + codeCounters.newTEMP();

    /*if (Symbols.isParameterClass(ctx->left_expr(0)->getText())) {
      code = code || instruction::LOAD(temp1, addrExpr1);
      addrExpr1 = temp1;
    }
    if (Symbols.isParameterClass(ctx->left_expr(1)->getText())) {
      code = code || instruction::LOAD(temp2, addrExpr2);
      addrExpr2 = temp2;
    }*/
    ...
    // Inicialitzar bucle
    // agafem la mesura del left expr y no mirem que siguin iguals.
    code = code || instruction::ILOAD(limit, std::to_string(Types.getArraySize(t0))); // Límit del bucle: mida de l'array
    code = code || instruction::ILOAD(ivar, "0"); // i = 0 
    code = code || instruction::ILOAD(inc, "1");  // increment es 1
    ...
    code = code || instruction::LT(cond, ivar, limit);          // cond = i < limit
    code = code || instruction::FJUMP(cond, labelEnd);
    code = code || instruction::LOADX(temp, addrExpr2, ivar);    //value = b[i]
    code = code || instruction::LOADX(tempX, addrExpr1, ivar);   //tempX = a[i]
    code = code || instruction::XLOAD(addrExpr2, ivar, tempX);    // b[i] = tempX
    code = code || instruction::XLOAD(addrExpr1, ivar, temp);    // a[i] = value
    code = code || instruction::ADD(ivar, ivar, inc);           // i++  ; i = i + inc
    code = code || instruction::UJUMP(labelLoop);
    code = code || instruction::LABEL(labelEnd);
  }
  // x = y
    ...
    code = code || instruction::LOAD(temp, addrExpr1);
    code = code || instruction::LOAD(addrExpr1, addrExpr2);
    code = code || instruction::LOAD(addrExpr2, temp);
  }
...
std::any CodeGenVisitor::visitSwitchStmt(AslParser::SwitchStmtContext *ctx){
  ...
  // Generate code for the switch expression (first expr)
  CodeAttribs     && codAtsE  = std::any_cast<CodeAttribs>(visit(ctx->expr(0)));
  std::string       addr_expr = codAtsE.addr;
  instructionList & code_expr = codAtsE.code;
  
  code = code_expr;

  // Generate labels for the switch
  std::string label = codeCounters.newLabelIF();
  std::string labelEndSwitch = "endswitch" + label;

  // Count case statements
  uint numCases = ctx->CASE().size();

  // Generate labels for each case
  std::vector<std::string> caseLabels;
  for (uint i = 0; i < numCases; ++i) {
    caseLabels.push_back("case" + label + "_" + std::to_string(i));
  }

  std::string defaultLabel = "default" + label;

  // Generate comparison code for each case
  for (uint i = 0; i < numCases; ++i) {
    // Generate code for case value (expr i+1 because expr 0 is the switch expression)
    CodeAttribs && codAtsCase = std::any_cast<CodeAttribs>(visit(ctx->expr(i+1)));
    std::string addr_case = codAtsCase.addr;
    instructionList & code_case = codAtsCase.code;
    
    code = code || code_case;
    
    // Compare switch expression with case value
    std::string temp_cmp = "%" + codeCounters.newTEMP();
    code = code || instruction::EQ(temp_cmp, addr_expr, addr_case);
    // Negate the result so we can use FJUMP to jump when NOT equal
    std::string temp_not = "%" + codeCounters.newTEMP();
    code = code || instruction::NOT(temp_not, temp_cmp);
    code = code || instruction::FJUMP(temp_not, caseLabels[i]);  // Jump to case if equal
  }
  
  // If no case matches, jump to default (if exists) or end
  if (ctx->DEFAULT()) {
    code = code || instruction::UJUMP(defaultLabel);
  } else {
    code = code || instruction::UJUMP(labelEndSwitch);
  }
  
  // Generate code for each case
  for (uint i = 0; i < numCases; ++i) {
    code = code || instruction::LABEL(caseLabels[i]);
    
    // Generate code for case statements
    instructionList && caseStmts = std::any_cast<instructionList>(visit(ctx->statements(i)));
    code = code || caseStmts;
  ...
  if (ctx->DEFAULT()) {
    code = code || instruction::LABEL(defaultLabel);
    // Default statements are the last ones
    instructionList && defaultStmts = std::any_cast<instructionList>(visit(ctx->statements(numCases)));
    code = code || defaultStmts;
  }
...
std::any CodeGenVisitor::visitLeft_expr(AslParser::Left_exprContext *ctx) {
  ...
  if(ctx->expr()){ // Si es tracta d'un array
    CodeAttribs     && codAtIdent = std::any_cast<CodeAttribs>(visit(ctx->ident()));
  ...
  // si es el parametro de una funcion, el que tenim es la adreça de l'array
  if (Symbols.isParameterClass(ctx->ident()->getText())) {
    std::string temp = "%"+codeCounters.newTEMP();
    code = code || instruction::LOAD(temp, addrIdent);
    addrIdent = temp;
  }
