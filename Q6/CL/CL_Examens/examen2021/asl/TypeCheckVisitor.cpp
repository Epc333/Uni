...
std::any TypeCheckVisitor::visitFunctionCall(AslParser::FunctionCallContext *ctx) {
  ...
  if (not Types.isErrorTy(tFunc) and not Types.isFunctionTy(tFunc)) {
    ...
    // ✅ Visitar TODOS los parámetros sin importar si hay errores
    ...
    // ✅ SIEMPRE visitar los parámetros, independientemente del número
    std::vector<TypesMgr::TypeId> paramTypes = Types.getFuncParamsTypes(tFunc);
    for (uint i = 0; i < ctx->expr().size(); ++i) {
      visit(ctx->expr(i)); // Esto visitará p**n y generará el error
      
      // Solo verificar compatibilidad si el número de parámetros es correcto
...
std::any TypeCheckVisitor::visitMapStmt(AslParser::MapStmtContext *ctx) {
  ...
  if (not Types.isErrorTy(t1) and not Types.isErrorTy(t2) and not Types.isErrorTy(t3)){

    if (not (Types.isArrayTy(t1) and Types.isArrayTy(t2) and Types.getArraySize(t1) == Types.getArraySize(t2)))
      Errors.incompatibleMapOperands(ctx);
    else if (not (Types.isFunctionTy(t3) and Types.getNumOfParameters(t3) == 1 and Types.getParameterType(t3, 0) == Types.getArrayElemType(t1) and Types.getFuncReturnType(t3) == Types.getArrayElemType(t2) ))
          Errors.incompatibleMapOperands(ctx);
...
// expr op=**  expr
// expr op=**  expr
std::any TypeCheckVisitor::visitPotencia(AslParser::PotenciaContext *ctx){
  DEBUG_ENTER();
  visit(ctx->expr(0));
  visit(ctx->expr(1));
  ...
  if ((not Types.isErrorTy(t0) and not Types.isNumericTy(t0)) or
