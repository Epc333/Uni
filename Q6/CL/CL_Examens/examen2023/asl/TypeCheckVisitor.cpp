...
std::any TypeCheckVisitor::visitForEachStmt(AslParser::ForEachStmtContext *ctx){
  ...
  else if (not Types.isErrorTy(t1) and not Types.isErrorTy(t0) and not Types.copyableTypes(t0,Types.getArrayElemType(t1)))
...
std::any TypeCheckVisitor::visitReduce(AslParser::ReduceContext *ctx){
  ...
  if (not Types.isErrorTy(t0) and not Types.isErrorTy(t1)){
    if (not Types.isArrayTy(t0))
      Errors.arrayIsRequired(ctx);
    else if (not Types.isFunctionTy(t1) or not(Types.isFunctionTy(t1) and Types.getNumOfParameters(t1) == 2 and Types.getFuncReturnType(t1) == Types.getParameterType(t1,0) and Types.getParameterType(t1,0) == Types.getParameterType(t1,1)))
      Errors.reduceInvalidFunction(ctx);
    else if (not (Types.getArrayElemType(t0) == Types.getFuncReturnType(t1)))
