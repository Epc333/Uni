...
int access_ok(int type, const void * addr, unsigned long size)
{
  ...
  switch (type) {
    ...
    case VERIFY_WRITE:
      ...
      //Zona heap: recorrer heap_slots
      if (!list_empty(&current()->heap_slots)) {
        list_for_each(pos, &current()->heap_slots) {
          hs = list_entry(pos, struct heap_slot, proc_list);

          if (addr_ini >= hs->start_page &&
              addr_fin < hs->start_page + hs->num_pages)
            return 1;
        }
      }
    ...
    default:
      // Zona codi + data
      if (addr_ini >= USER_FIRST_PAGE &&
          addr_fin < PAG_LOG_INIT_CODE + NUM_PAG_CODE)
        return 1;
