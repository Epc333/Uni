...
//Estructura per emmagatzemar informació sobre els slots
struct heap_slot heap_slot_vec[HEAP_SLOTS];
...
void init_heap_slots() {
    for (int i = 0; i < HEAP_SLOTS; ++i) {
        heap_slot_vec[i].owner = NULL;
        heap_slot_vec[i].start_page = 0;
        heap_slot_vec[i].num_pages = 0;
        INIT_LIST_HEAD(&heap_slot_vec[i].proc_list);
    }
}
